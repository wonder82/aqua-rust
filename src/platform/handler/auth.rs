...
pub async fn register(State(state): State<SharedState>, headers: HeaderMap, body: Bytes) -> Response {
    ...
    if email.is_empty() || username.is_empty() || password.is_empty() || code.is_empty() {
        return write_err(StatusCode::BAD_REQUEST, "invalid_request", "请填写所有必填项");
    }
    let (allowed, reason) = guard::is_allowed_domain(&email);
    if !allowed {
        return write_err(StatusCode::BAD_REQUEST, "email_not_allowed", &reason);
    ...
    // 注册协议仿冒检测
    let device_fp = headers.get("X-Device-Fingerprint").and_then(|v| v.to_str().ok()).unwrap_or("").to_string();
    let ua = headers.get(axum::http::header::USER_AGENT).and_then(|v| v.to_str().ok()).unwrap_or("").to_string();
    let accept = headers.get(axum::http::header::ACCEPT).and_then(|v| v.to_str().ok()).unwrap_or("").to_string();
    let accept_lang = headers.get("accept-language").and_then(|v| v.to_str().ok()).unwrap_or("").to_string();
    let ip = client_ip(&headers, "");
    if let Some(msg) = guard::REGISTRATION_GUARD.validate(&ua, &accept, &accept_lang, &device_fp, &ip) {
        return write_err(StatusCode::FORBIDDEN, "registration_blocked", &msg);
    }
    // 随机用户名检测
    ...
    if password.chars().count() < 8 {
        return write_err(StatusCode::BAD_REQUEST, "invalid_request", "密码长度不能少于8位");
    }
    // 验证验证码
    ...
    let exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM users WHERE email=$1 AND status='active' OR username=$2 AND status='active')",
    )
    .bind(&email)
    .bind(&username)
    .fetch_one(&state.pool)
    ...
    .unwrap_or(false);
    if exists {
        return write_err(StatusCode::CONFLICT, "exists", "该邮箱或用户名已被注册");
    }
    // 哈希密码
    ...
    // 创建用户（gw_client_id 取 client_id_seq 数字自增）
    ...
    let (user_id, gw_client_id) = match row {
        ...
        Err(e) => {
            ...
            return write_err(StatusCode::INTERNAL_SERVER_ERROR, "server_error", "创建用户失败，请稍后重试");
        }
    };
    // 创建网关客户端记录
    ...
    let mut resp = write_ok(StatusCode::CREATED, json!({"user_id": user_id, "username": username, "email": email}));
