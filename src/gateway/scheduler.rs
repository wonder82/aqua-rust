...
impl SurgeScheduler {
    ...
    pub async fn load_active_keys(&self) -> Vec<UpstreamKey> {
        ...
        let rows = sqlx::query_as::<_, (String, String, String, String, String, i32, i32, i32, String, String)>(
            "SELECT id, name, provider, api_key_ciphertext, key_prefix, weight, rpm_limit, switch_threshold, model_scope, status \
        ...
        let keys: Vec<UpstreamKey> = rows
            .into_iter()
            .map(|(id, name, provider, api_key_ciphertext, key_prefix, weight, rpm_limit, switch_threshold, model_scope, status)| {
                UpstreamKey {
                    id,
                    name,
                    provider,
